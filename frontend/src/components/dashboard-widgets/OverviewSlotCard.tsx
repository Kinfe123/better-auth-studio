...
export function OverviewSlotCard({ children, className = "" }: OverviewSlotCardProps) {
  return (
    <div
      className={`bg-gradient-to-b from-white/[4%] to-white/[2.5%] border border-white/10 rounded-none p-4 md:p-6 relative h-full flex flex-col ${className}`}
    >
      ...
      <div className="flex-1 min-h-0 flex flex-col pt-4">{children}</div>
    </div>
  );
}
