...
import { AlertInfo, ErrorInfo, Info } from '../components/PixelIcons';
...
export default function Events() {
    ...
    const eventStats = useMemo(() => {
        ...
        const failed = events.filter((e) => e.status === 'failed' || e.display?.severity === 'failed')
            .length;
        const warning = events.filter((e) => e.display?.severity === 'warning').length;
        const info = events.filter((e) => e.display?.severity === 'info' || !e.display?.severity)
            .length;
        ...
        try {
            ...
            if (data.events && Array.isArray(data.events)) {
                ...
                if (isInitial) {
                    ...
                    // Polling - only add truly new events (not already in our list)
                    setEvents((prev) => {
                        ...
                        <div className="space-y-6 mt-4">
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    ...
                                    {selectedEvent.status === 'success' && selectedEvent.display?.severity !== 'failed' && (
                                        <div
                                    ...
                                    {(selectedEvent.status === 'failed' || selectedEvent.display?.severity === 'failed') && (
                                        <div
                                            className="absolute inset-0 pointer-events-none opacity-[8%]"
                                            style={{
                                                backgroundImage: `repeating-linear-gradient(0deg, rgba(239, 68, 68, 0.3), rgba(239, 68, 68, 0.3) 1px, transparent 1px, transparent 4px)`,
                                    ...
                                    <div className="relative z-10">
                                        {getEventIcon(selectedEvent.type, selectedEvent.display?.severity, selectedEvent.status)}
                                    </div>
                                </div>
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="mt-2">
                                    <CopyableId id={selectedEvent.id} variant="detail" />
                                </div>
