...
${middleware.length > 0 || hooks.some((h: any) => h.action === 'custom') ? 'import { sessionMiddleware } from "better-auth/api";' : ''}
...
${hooks.length > 0 ? '\n  // Hooks configuration\n' : ''}${hooks
  .map(
    (hook: any) =>
      `  ${hook.name}?: (ctx: GenericEndpointContext) => Promise<void> | void;`
  )
  .join('\n')}
...
${hooks.filter((h: any) => h.action === 'custom' && h.customPath).map((hook: any, idx: number) => {
...
${table.fields?.map((field: any) => `            ${field.name}: {
  ...
  .join('\n\n')}
    ...
    ${middleware.length > 0 || hooks.some((h: any) => h.action === 'custom' && h.customPath) ? `endpoints: {
${hooks.filter((h: any) => h.action === 'custom' && h.customPath).map((hook: any, idx: number) => {
  const endpointName = hook.name.trim() ? hook.name.toLowerCase().replace(/[^a-zA-Z0-9]/g, '') : `customHook${idx + 1}`;
  return `      ${endpointName}: customHookEndpoints["${endpointName}"],`;
}).join('\n')}
${middleware.length > 0 ? '      ' : ''}${middleware
  .map((mw: any, idx: number) => {
  ...
  .join('\n      ')}
    ...
    ${hooks.length > 0 ? `init(ctx) {
      return {
        options: {
          databaseHooks: {
            ${hooks.some((h: any) => h.action === 'sign-up') ? `user: {
              create: {
                ${hooks.filter((h: any) => h.action === 'sign-up').map((hook: any) => {
                  return `${hook.timing}: {
            ...
            ${hooks.some((h: any) => h.action === 'sign-in') ? `session: {
              create: {
                ${hooks.filter((h: any) => h.action === 'sign-in').map((hook: any) => {
                  return `${hook.timing}: {
...
${tables.map((table: any) => {
  return `      ${table.name}: {
        ...
        createdAt: "date",
        updatedAt: "date",
${table.fields?.map((field: any) => `        ${field.name}: "${field.type}",`).join('\n') || ''}
      },`;
}).join('\n')}
    },` : ''}
    ${rateLimit ? `rateLimit: {
